...
//             this.geneName.add(gene.get(0).split("\t")[0].split(">")[0].split(".")[0]);
             if (gene.get(1).length() > 350){
                 ...
                 this.filterGeneName.add(gene.get(0).split("\t")[0]);
...
//        }
    ...
    public static ArrayList<String> randomSequence(ArrayList<ArrayList<String>> filterBlock, int startPos){
        ArrayList<String> res = new ArrayList<>();
//        ArrayList<Integer> startPos = new ArrayList<>();
//        for (ArrayList<String> gene: filterBlock) {
//            int p = getRandom(0, gene.get(1).length()-351);
...
//        }
        for (int i = 0; i < filterBlock.size(); i++) {
            res.add((String) filterBlock.get(i).get(1).subSequence(startPos, startPos+350));
        }
        return res;
    }
    ...
    public static String random(String seq){
        StringBuilder sb = new StringBuilder(seq);
        for (int i = 0; i < seq.length(); i++) {
            double p = Math.random();
            if (p <= 0.0025){ sb.setCharAt(i, 'A'); }
            else if (p <= 0.005){sb.setCharAt(i, 'T');}
            else if (p <= 0.0075){sb.setCharAt(i, 'C');}
            else if (p <= 0.01){sb.setCharAt(i, 'G');}
        }
        seq=sb.toString();
        return seq;
    }
